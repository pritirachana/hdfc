app.controller("irController", function($scope, $rootScope, $state, $compile, $window, $socket, $timeout,gratification,$sce) {

    $scope.broucherDownload = function(url){
        gratification.sendPoints('user_self_activity','ir_booth_broucher','IR');
        $window.open(url, '_blank');
    }

    gratification.sendPoints('user_self_activity','ir_booth_enter','IR');
    ...
    $scope.hideVideoSolution = function(){
        ...
        setInterval(function () {
            $socket.emitIndividualExhibitionChat("avaya_on_exhibition", {"user_data" : $rootScope.currentUser, "stall_id" : 4});
          }, 1000);


        $socket.emitIndividualExhibitionChat("avaya_on_exhibition", {"user_data" : $rootScope.currentUser, "stall_id" : 4});
   
       $socket.emitIndividualExhibitionChat("avaya_fetch_history", {"user_id" : $rootScope.currentUser.id, "exhibitor_id" : 4});


       $socket.on("avaya_exhi_user_chat_history", function (data) {
                // console.log(data);
                if (data.data.user_id ==  $rootScope.currentUser.id && data.data.exhibitor_id == 4) {
       ...
       $scope.$on("$destroy", function(){
          ...
          $socket.emitIndividualExhibitionChat("avaya_off_exhibition", {"user_data" : $rootScope.currentUser, "stall_id" : 4});
       });
       ...
       $socket.on("avaya_new_msgs_from_exhibitor", function(data) {
                ...
                if (data.user_id == $rootScope.currentUser.id && data.exhibitor_id == 4) {
        ...
        $scope.sendMessage = function(message) {
            ...
            if (message != '') {
                ...
                var today = new Date();
                var time = today.getHours() + ":" + today.getMinutes();
                $socket.emitIndividualExhibitionChat("avaya_new_msgs_user", {"user_data": $rootScope.currentUser, "exhibitor_id" : 4, "message" : message});
