app.controller("serviceController", function($scope, $rootScope, $state, $compile, $window, $socket, $timeout,$http,gratification,$sce) {
    ...
    //     gratification.sendPoints('user_self_activity','avaya_booth_pro_ser_complete_intro_video');
    // });
    ...
    //         // player.play();
    //     player.on('play', function() {
    //         gratification.sendPoints('user_self_activity','avaya_booth_service_solution_start_intro_video');
    //     });
    //     player.on('ended', function(data) {
    //         console.log('Video Ended');
    //         gratification.sendPoints('user_self_activity','avaya_booth_service_solution_complete_intro_video');
    //     });
    ...
    //     meeting_data.booth = 'avaya_one_services';
    ...
    $scope.broucherDownload = function(url){
        gratification.sendPoints('user_self_activity','avaya_booth_service_brouchers');
    ...
    $scope.fetchQuiz = function(){
        $http({
            ...
            params:{booth:'avaya_one_services'}
        }).then(function(data){
            if(data.data.status == true){
    ...
    $scope.submitQuestion = function(question_answer){
        ...
        user_answer.booth = 'avaya_one_services';
        ...
        setInterval(function () {
            $socket.emitIndividualExhibitionChat("avaya_on_exhibition", {"user_data" : $rootScope.currentUser, "stall_id" : 18});
        }, 1000);


        $socket.emitIndividualExhibitionChat("avaya_on_exhibition", {"user_data" : $rootScope.currentUser, "stall_id" : 18});
   
       $socket.emitIndividualExhibitionChat("avaya_fetch_history", {"user_id" : $rootScope.currentUser.id, "exhibitor_id" : 18});


       $socket.on("avaya_exhi_user_chat_history", function (data) {
                console.log(data);
                if (data.data.user_id == $rootScope.currentUser.id && data.data.exhibitor_id == 18) {
                    console.log("test");
       ...
       $scope.$on("$destroy", function(){
          ...
          $socket.emitIndividualExhibitionChat("avaya_off_exhibition", {"user_data" : $rootScope.currentUser, "stall_id" : 18});
       });
       ...
       $socket.on("avaya_new_msgs_from_exhibitor", function(data) {
                ...
                if (data.user_id == $rootScope.currentUser.id && data.exhibitor_id == 18) {
        ...
        $scope.sendMessage = function(message) {
            ...
            if (message != '') {
                ...
                var today = new Date();
                var time = today.getHours() + ":" + today.getMinutes();
                $socket.emitIndividualExhibitionChat("avaya_new_msgs_user", {"user_data": $rootScope.currentUser, "exhibitor_id" : 18, "message" : message});
