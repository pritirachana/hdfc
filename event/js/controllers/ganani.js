app.controller("gananiController", function($scope, $rootScope, $state, $compile, $window, $socket, $timeout,gratification,$sce) {

    $scope.broucherDownload = function(url){
        gratification.sendPoints('user_self_activity','gnani_booth_broucher','Ganani');
        $window.open(url, '_blank');
    }

    gratification.sendPoints('user_self_activity','gnani_booth_enter','Ganani');
    ...
    setInterval(function () {
        $socket.emitIndividualExhibitionChat("avaya_on_exhibition", {"user_data" : $rootScope.currentUser, "stall_id" : 2});
      }, 1000);

    $socket.emitIndividualExhibitionChat("avaya_on_exhibition", {"user_data" : $rootScope.currentUser, "stall_id" : 2});
   
       $socket.emitIndividualExhibitionChat("avaya_fetch_history", {"user_id" : $rootScope.currentUser.id, "exhibitor_id" : 2});


       $socket.on("avaya_exhi_user_chat_history", function (data) {
                // console.log(data);
                if (data.data.user_id ==  $rootScope.currentUser.id && data.data.exhibitor_id == 2) {
       ...
       $scope.$on("$destroy", function(){
           ...
           $socket.emitIndividualExhibitionChat("avaya_off_exhibition", {"user_data" : $rootScope.currentUser, "stall_id" : 2});
       });
   
       $socket.on("avaya_new_msgs_from_exhibitor", function(data) {
                console.log(data);
                
                if (data.user_id == $rootScope.currentUser.id && data.exhibitor_id ==2) {
                ...
                var divElement = angular.element(document.querySelector('.message-container'));
                console.log(data.user_id == $rootScope.currentUser.id && data.exhibitor_id ==2);
                   var today = new Date();
        ...
        $scope.sendMessage = function(message) {
            ...
            if (message != '') {
                ...
                var today = new Date();
                var time = today.getHours() + ":" + today.getMinutes();
                $socket.emitIndividualExhibitionChat("avaya_new_msgs_user", {"user_data": $rootScope.currentUser, "exhibitor_id" : 2, "message" : message});
