app.controller("extreme-networksController", function($scope, $rootScope, $state, $compile, $window, $socket, $timeout,gratification,$sce) {

    $scope.broucherDownload = function(url){
        gratification.sendPoints('user_self_activity','extreme_booth_broucher','Extreme Networks');
        $window.open(url, '_blank');
    }

    gratification.sendPoints('user_self_activity','extreme_booth_enter','Extreme Networks');
    ...
    $scope.hidechatWithUs = function(){
      ...
      setInterval(function () {
            $socket.emitIndividualExhibitionChat("avaya_on_exhibition", {"user_data" : $rootScope.currentUser, "stall_id" : 10});
        }, 1000);


     $socket.emitIndividualExhibitionChat("avaya_on_exhibition", {"user_data" : $rootScope.currentUser, "stall_id" : 10});
   
       $socket.emitIndividualExhibitionChat("avaya_fetch_history", {"user_id" : $rootScope.currentUser.id, "exhibitor_id" : 10});


       $socket.on("avaya_exhi_user_chat_history", function (data) {
                // console.log(data);
                if (data.data.user_id ==  $rootScope.currentUser.id && data.data.exhibitor_id == 10) {
       ...
       $scope.$on("$destroy", function(){
           ...
           $socket.emitIndividualExhibitionChat("avaya_off_exhibition", {"user_data" : $rootScope.currentUser, "stall_id" : 10});
       });
       ...
       $socket.on("avaya_new_msgs_from_exhibitor", function(data) {
                ...
                console.log(data.user_id == $rootScope.currentUser.id && data.exhibitor_id ==10);
