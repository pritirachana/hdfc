app.controller("ccaasController", function($scope, $rootScope, $state, $compile, $window, $socket, $timeout,$http,gratification,$sce) {
    ...
    $scope.startVideo = function(){
        ...
        player.on('play', function() {
            gratification.sendPoints('user_self_activity','avaya_booth_ccaas_start_intro_video');
        });
        player.on('ended', function(data) {
            console.log('Video Ended');
            gratification.sendPoints('user_self_activity','avaya_booth_ccaas_complete_intro_video');
        });
    ...
    $scope.submitMeeting = function(meeting_data){
        ...
        meeting_data.booth = 'avaya_one_cloud_ccass';
    ...
    $scope.submitFeedback = function(feedback){
        ...
        feedback.booth = 'avaya_one_cloud_ccass';
    ...
    $scope.fetchQuiz = function(){
        $http({
            ...
            params:{booth:'avaya_one_cloud_ccass',user_id:$rootScope.currentUser.id}
        }).then(function(data){
            if(data.data.status == true){
    ...
    $scope.broucherDownload = function(url){
        gratification.sendPoints('user_self_activity','avaya_booth_ccaas_brouchers');
    ...
    $scope.submitQuestion = function(question_answer){
        ...
        if($scope.quiz_questions[$scope.current_question_number]['user_answer']){
            ...
            user_answer.booth = 'avaya_one_cloud_ccass';
        ...
        setInterval(function () {
            $socket.emitIndividualExhibitionChat("avaya_on_exhibition", {"user_data" : $rootScope.currentUser, "stall_id" : 15});
        }, 1000);


        $socket.emitIndividualExhibitionChat("avaya_on_exhibition", {"user_data" : $rootScope.currentUser, "stall_id" : 15});
   
       $socket.emitIndividualExhibitionChat("avaya_fetch_history", {"user_id" : $rootScope.currentUser.id, "exhibitor_id" : 15});


       $socket.on("avaya_exhi_user_chat_history", function (data) {
                console.log(data);
                if (data.data.user_id == $rootScope.currentUser.id && data.data.exhibitor_id == 15) {
       ...
       $scope.$on("$destroy", function(){
          ...
          $socket.emitIndividualExhibitionChat("avaya_off_exhibition", {"user_data" : $rootScope.currentUser, "stall_id" : 15});
       });
       ...
       $socket.on("avaya_new_msgs_from_exhibitor", function(data) {
                ...
                if (data.user_id == $rootScope.currentUser.id && data.exhibitor_id == 15) {
        ...
        $scope.sendMessage = function(message) {
            ...
            if (message != '') {
                ...
                var today = new Date();
                var time = today.getHours() + ":" + today.getMinutes();
                $socket.emitIndividualExhibitionChat("avaya_new_msgs_user", {"user_data": $rootScope.currentUser, "exhibitor_id" : 15, "message" : message});
