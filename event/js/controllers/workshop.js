app.controller("workshopController", function($scope,$http, $location, $rootScope, $state,$interval, $compile, $window, $socket, $timeout,gratification,$sce,$timeout) {   
   ...
   $http.post('https://devapi.virtex.in/api/cii-bio/get-session',{stageType:1}).then(function(result){
    console.log(result);
   ...
   $scope.currentSession = function(session){
      ...
      console.log(session.vod_link);
      $(".stage-player").empty().append(session.vod_link);
      // $state.go('auditorium');
