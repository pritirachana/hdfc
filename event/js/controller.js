app.controller("mainController", function($scope, $http, $state, $rootScope, $window, $socket, $localStorage, $compile, $filter) {
    ...
    $rootScope.urlprefix = 'http://35.154.120.192/apiavaya';
    ...
    $rootScope.getUserName = function(first_name, last_name) {
        return first_name.charAt(0).toLowerCase();
    }
...
app.controller("isaController", function($scope, $rootScope, $state, $compile, $window, $socket, $timeout) {
...
app.controller("ceoController", function($scope, $rootScope, $state, $compile, $window, $socket, $timeout) {



});
app.controller("stage2Controller", function($scope, $rootScope, $state, $compile, $window, $socket, $timeout) {
...
app.controller("briefcaseController", function($scope, $rootScope, $state, $compile, $window, $socket, $timeout) {
...
app.controller("speakerController", function($scope, $rootScope, $state, $compile, $window, $socket, $timeout) {



});
app.controller("lobbyController", function($scope, $rootScope, $state, $compile, $window, $socket, $timeout) {
...
app.controller("supportController", function($scope, $rootScope, $state, $compile, $window, $socket, $timeout) {
...
app.controller("videoConfController", function($scope, $rootScope, $state, $compile, $window, $socket, $timeout) {
    ...
    var apiKey = "46882104";
    var sessionId = "2_MX40Njg4MjEwNH5-MTU5Nzc1NzYzNjA1M35sVi9vWlFvVlBZMU93cjJISEM1YWxiaGl-fg";
    var token = "T1==";
...
app.controller("networkingController", function($scope, $rootScope, $state, $compile, $window, $socket) {
    ...
    $socket.on('returnonlineusers', function(data) {
        ...
        for (const [key, value] of Object.entries(data)) {
            ...
            // var first_val = $rootScope.getUserName(user.first_name, user.last_name);
            divNetElement.append($compile(angular.element('<div id="net-people-data3"><img class="net-people-img3" src="images/flags/' + user.country + '.svg"  alt="Avatar" ><p class="net-people-name3">' + user.first_name + '</p><button id="people-chat-button" ng-click=startVideosChat()><i class="fas fa-video">&nbsp; &nbsp; </i>Invite to call</button></div>'))($scope));
        }
    });
    ...
    if (angular.isDefined($rootScope.currentUser)) {
        ...
        $scope.sendMessage = function(message) {
            ...
            if (message != '') {
                ...
                let comment = angular.element('<div class="chat-box"><div class="chat-img"><img id="username-image" src="images/flags/' + $rootScope.currentUser.country + '.svg" class="user-menu_avatar__1ZmG9 user-menu_border__2k_pD" alt="avatar"></div><div class="chat-text"><a href="#"><span class="chat-name">' + $rootScope.currentUser.first_name + '</span></a><span class="chat-time">' + time + '</span><div class="chat-para"><p>' + message + '</p></div></div></div>');
                divElement.append(comment);
                $compile(divElement)($scope);
                angular.element(divElement)[0]['scrollTop'] = divElement[0]['scrollHeight'];
            }
...
app.controller("pollsController", function($scope, $rootScope, $state, $socket, $compile, $window) {
    ...
    $socket.on('showquestion', function(data) {
        ...
        for (var i = 0; i < total_value; i++) {
            ...
            $rootScope.all_question_ans_data[0] = user_data[i];
    ...
    $socket.on('pollanswer', function(data) {
        ...
        console.log(data);
        console.log($rootScope.all_question_ans_data);
        if ($rootScope.all_question_ans_data && $rootScope.all_question_ans_data[0] && $rootScope.all_question_ans_data[0]['question'] == data.question) {

            var total = ((parseInt(data.val[0]) ? parseInt(data.val[0]) : 0) + (parseInt(data.val[1]) ? parseInt(data.val[1]) : 0) + (parseInt(data.val[2]) ? parseInt(data.val[2]) : 0) + (parseInt(data.val[3]) ? parseInt(data.val[3]) : 0));
            let first_value = (parseInt(data.val[0]) ? parseInt(data.val[0]) : 0);
            let first_per = (first_value / total) * 100;
            let second_value = (parseInt(data.val[1]) ? parseInt(data.val[1]) : 0);
            let second_per = (second_value / total) * 100;
            let third_value = (parseInt(data.val[2]) ? parseInt(data.val[2]) : 0);
            let third_per = (third_value / total) * 100;
            let fourth_value = (parseInt(data.val[3]) ? parseInt(data.val[3]) : 0);
            let fourth_per = (fourth_value / total) * 100;
            $rootScope.all_question_ans_data[0]['first_per'] = first_per;
            $rootScope.all_question_ans_data[0]['second_per'] = second_per;
            $rootScope.all_question_ans_data[0]['third_per'] = third_per;
            $rootScope.all_question_ans_data[0]['fourth_per'] = fourth_per;
    ...
    $scope.submitAnswer = function(poll, key, val) {
        console.log(poll);
        if (!poll.completed) {
            poll.completed = 1;
            var data = {};
            ...
            data.answers = val;
            console.log(data);
            $socket.emit('questionsanswers', data);
        }
...
app.controller("peopleController", function($scope, $rootScope, $state, $socket, $compile, $window) {
    ...
    $scope.sendMsg = function() {
        ...
        if (msg != "" || msg != " ") {
            ...
            let comment = angular.element('<div id="people-chat-text"><div class="chat-text2"><a href="#"><span class="people-chat-name">' + $rootScope.currentUser.first_name + '</span></a><span class="people-chat-time">' + time + '</span></div><div class="people-chat-para"><p>' + msg + '</p></div></div>');
            divElement.append(comment);
            $compile(divElement)($scope);
            var user_name = $rootScope.currentUser.first_name;
    ...
    $socket.on("new_msg", function(data) {
        ...
        var divElement = angular.element(document.querySelector('#people-chat'));
        let comment = angular.element('<div id="people-chat-text"><div class="chat-text2"><a href="#"><span class="people-chat-name">' + data.user_name + '</span></a><span class="people-chat-time">'+time+'</span></div><div class="people-chat-para"><p>' + data.message + '</p></div></div>');
        divElement.append(comment);
        $compile(divElement)($scope);
